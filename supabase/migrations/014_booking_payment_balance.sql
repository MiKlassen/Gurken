...
alter table public.bookings
  drop constraint if exists bookings_paid_amount_valid;

alter table public.bookings
  add constraint bookings_paid_amount_valid check (paid_amount_cents >= 0);
...
set text_body = replace(
    ...
    html_body = replace(
      html_body,
      '<tr><td>Betrag</td><td><strong>{{amount}}</strong></td></tr><tr><td>Status</td><td>{{status}}</td></tr>',
      '<tr><td>Betrag</td><td><strong>{{amount}}</strong></td></tr><tr><td>Zahlungsstand</td><td>{{paymentBalance}}</td></tr><tr><td>Status</td><td>{{status}}</td></tr>'
    )
where key = 'booking_confirmation'
  and updated_by is null
  and text_body not like '%{{paymentBalance}}%';
...
set text_body = replace(
    ...
    html_body = replace(
      html_body,
      '<tr><td>Betrag</td><td><strong>{{amount}}</strong></td></tr></table>',
      '<tr><td>Betrag</td><td><strong>{{amount}}</strong></td></tr><tr><td>Zahlungsstand</td><td>{{paymentBalance}}</td></tr></table>'
    )
